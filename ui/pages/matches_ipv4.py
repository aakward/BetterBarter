...
from services import matching_ipv4 as matching
...
def main():
    ...
    if matches:
        for idx, m in enumerate(matches):
            with st.expander(f"Match {idx+1}: {m['offer_title']} ↔ {m['request_title']}"):
                st.markdown(
                    f"**Offer:** {m['offer_title']} (User: {m['offer_user_name']}, Postal Code: {m['offer_postal']})"
                )
                st.markdown(
                    f"**Request:** {m['request_title']} (User: {m['request_user_name']}, Postal Code: {m['request_postal']})"
                )
    ...
    if sent_requests:
        for r in sent_requests:
            offer_title = r.get("offer_title") if r.get("offer_title") else "—"
            request_title = r.get("request_title") if r.get("request_title") else "—"
            status = r.get("status") or "pending"
            st.markdown(f"- Offer: **{offer_title}** → Request: **{request_title}** | Status: **{status}**")
            if r.get("message"):
    ...
    user_offers = crud.get_all_offers(db, exclude_profile_id=None)
    incoming_requests = []
    for offer in user_offers:
        if offer["profile_id"] == profile_id:
            requests_for_offer = crud.get_match_requests_for_offer(db, offer_id=offer["id"])
            if requests_for_offer:
                for r in requests_for_offer:
                    r["offer_data"] = offer  # attach offer info for display
                    incoming_requests.append(r)

    if incoming_requests:
        for r in incoming_requests:
            requester_name = r.get("requester_name") or "Unknown"
            offer_title = r.get("offer_data", {}).get("title") or "—"
            st.markdown(f"**From {requester_name} for your offer '{offer_title}'**")
            if r.get("message"):
                st.caption(f"Message: {r['message']}")

            col1, col2 = st.columns(2)
            if col1.button("✅ Approve", key=f"approve_{r['id']}"):
                crud.update_match_request_status(db, r["id"], "accepted")
