...
SUPABASE_URL = st.secrets["SUPABASE_URL"] #os.getenv("SUPABASE_URL")
SUPABASE_ANON_KEY = st.secrets["SUPABASE_ANON_KEY"] #os.getenv("SUPABASE_ANON_KEY")
...
def main():
        ...
        with st.form("login_form"):
            ...
            if submitted:
                if not email or not password:
                    ...
                    user = crud.authenticate_user(supabase, email=email, password=password)
                    if user:
                        # user.id is Supabase UUID
                        auth.login_user(user.id)
                        profile = db.query(crud.models.Profile).filter(crud.models.Profile.id == user.id).first()
    ...
    with tab2:
        ...
        with st.form("register_form"):
            ...
            phone = st.text_input("WhatsApp Number (Phone Numbers would only be shared with matches once you approve of a match request. " \
            "We will notify you via email when you have a match.)", placeholder="+31101234567")
            share_phone = st.checkbox("Share phone with matches without waiting for approval?", value=False)
